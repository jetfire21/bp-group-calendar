...
?>
			...
			<?php $for_printer = false;
			$table_head = '
			<table id="a21_bgc_tasks_shifts" data-event-id="<?php echo $event_id;?>" style="margin-bottom: 5px;">
	        	<tr class="title_columns">
	        	   <th class="a21_dinam_th_coll"> Task item </th>';
		        	foreach ($event_times as $k => $v):
	        		$table_head .= '<th class="a21_dinam_th_coll">'.$v->time.'</th>';
			        endforeach;
		       $table_head .= '
		       </tr>';
	        	foreach ($event_tasks as $k => $task):
	        	...
	        	$for_site .= '
		        <tr class="a21_dinam_row" data-task_id="'.$task->id.'">
		        	<td class="a21_dinam_coll">'.$task->task_title.'</td>';

	        		 foreach ($task->cnt_vols as $k2 => $cnt):
	        			...
	        			// this array can have empty values ""
		        		 ...
		        		 $for_site .= '<td class="a21_dinam_coll '; if($cur_count >= $cnt) $for_site .= 'red-cell'; 
		        		 if($cur_count > 0 && $cur_count != $cnt) $for_site .= 'yellow-cell';
		        		 $for_site .= ' ">'; 
						 ...
						 if($cur_count >= $cnt) $for_site .= "<p>Awesome! this task is FULL</p>";
 		        		 if( $still_need_count != 0 ) { $for_site .= "<p class='still_need'><span class='vol_cnt'>".$still_need_count."</span> ".$need."</p>"; }
						...
						/* **** hide/show btn signup **** */
		        		  ...
		        		  $for_printer = $for_site;

		        		 // echo '<br>vol_hide_btn ='; var_dump($vol_hide_btn);
		        		 if(!$vol_hide_btn && is_user_logged_in() ):
		        		 
			        		 $for_site .= '<button class="a21_add_new_volunteer" data-s-need-cnt="'.$still_need_count.'" data-i="'.$k2.'" data-id="'.$cur_user->ID.'" data-nick="'.$cur_user->data->user_nicename.'">i will volunteer</button>';
		        		 endif;
		        		 if(!$vol_hide_btn && !is_user_logged_in() ):
			        		 $for_site .= '<button class="a21_bgc_user_signup">signup</button>';
		        		 endif;
						...
						*/
		        		  ...
		        		  if( !empty($show_cancel_my_attandance) ) $for_site .= $show_cancel_my_attandance;
		        		  $member_name = '';
		        		  $i = 1;
		        		  foreach ($arr_vols_cur_task as $member_id) {
	  	        		      if(!empty($member_id) ) {
	  	        		      	if($i == 1) $html_member .= "<p class='attendees'>DuGoodrs: ";
								...
								$html_member .= "<a class='link-user-id-".$member_id."' href='".$member_link."' >".$member_avatar."</a>";
								if($i == count($arr_vols_cur_task)) $html_member .= "</p>";
								$i++;  	        		     
							 }
		        		  }
		        		  // it that hide all buttons for printer
		        		  $for_site .= $html_member.'</td>';
		        		  $for_printer .= $html_member.'</td>';
		        		  // echo $member_name;
		      		 endforeach;
    	        $for_site .= $for_site.'</tr>';
    	        $for_printer .= $for_printer.'</tr>';
		        endforeach;
			$for_site .= $for_site .'</table>';
			$for_printer .= $for_printer .'</table>';
			?>
